...
class AppCityAgent_Agents implements INode {
  ...
  constructor(fields?: { sessionId?: string }) {
    ...
    this.description =
      "Orchestrator agent that coordinates between a Sales Agent and an ERP Agent with configurable thinking order";
    ...
    this.inputs = [
      {
        ...
        type: "AgentExecutor",
        description:
          "The Sales Agent (Tool Agent) that handles customer-facing conversations",
      },
      {
        label: "ERP Agent",
        name: "erpAgent",
        ...
        description:
          "The ERP Agent that makes data-driven decisions and interacts with ERP systems",
      },
      {
        label: "Shared Memory",
        name: "memory",
        type: "BaseChatMemory",
        description:
          "Shared memory between both agents — accepts all memory node types",
      },
      {
        ...
        type: "BaseChatModel",
        description:
          "Chat model used by the orchestrator to merge responses and make routing decisions",
      },
      {
        ...
        options: [
          {
            ...
            name: "salesFirst",
            description:
              "Sales Agent responds first, then ERP Agent reviews and enriches the response",
          },
          {
          ...
          },
          {
            ...
            name: "parallel",
            description:
              "Both agents think simultaneously, then the orchestrator merges their outputs",
          },
        ],
        default: "erpFirst",
      },
      {
        ...
        description:
          "Instructions for the orchestrator on how to merge and coordinate agent responses",
        rows: 4,
        optional: true,
      ...
      },
      {
        ...
        description:
          "Detect text that could generate harmful output and prevent it from being sent to the agents",
  ...
  async run(
    ...
    const thinkingOrder =
      (nodeData.inputs?.thinkingOrder as string) || "erpFirst";
    const systemMessage =
      (nodeData.inputs?.systemMessage as string) || DEFAULT_ORCHESTRATOR_PROMPT;

    const shouldStreamResponse = options.shouldStreamResponse;
    const sseStreamer: IServerSideEventStreamer =
      options.sseStreamer as IServerSideEventStreamer;
    const chatId = options.chatId;

    const loggerHandler = new ConsoleCallbackHandler(
      options.logger,
      options?.orgId
    );
    ...
    const chatHistory = (await memory.getChatMessages(
      this.sessionId,
      true,
      prependMessages
    )) as BaseMessage[];
    ...
    if (thinkingOrder === "salesFirst") {
      ...
      allUsedTools.push(
        ...(salesResult.usedTools || []),
        ...(erpResult.usedTools || [])
      );
      allSourceDocuments.push(
        ...(salesResult.sourceDocuments || []),
        ...(erpResult.sourceDocuments || [])
      );
      allArtifacts.push(
        ...(salesResult.artifacts || []),
        ...(erpResult.artifacts || [])
      );
      ...
      if (erpResponseMode === "directResponse") {
        ...
        // decisionOnly — Sales Agent output is the final response
        // ERP ran its tools (monitoring, etc.) as a side-effect
        finalOutput = salesResult.output;
      }
    }
  ...
  private async runAgent(
    ...
    const agentCallbacks =
      shouldStream && sseStreamer
        ? [
            loggerHandler,
            new CustomChainHandler(sseStreamer, chatId),
            ...callbacks,
          ]
