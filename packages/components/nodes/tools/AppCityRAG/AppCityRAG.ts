...
class AppCityRAG_Tools implements INode {
  ...
  constructor() {
    ...
    this.baseClasses = [this.type, ...getBaseClasses(AppCityRAGTool), "Tool"];
    this.inputs = [
      {
        label: "ERP Base URL",
        name: "erpBaseUrl",
      ...
      },
      {
        ...
        options: [
          {
            label: "All Matches (return all matching rows)",
            name: "all_matches",
          },
    ...
    listSheets: async (
      ...
      if (crmApiKey) {
        try {
            ...
            return [
              {
                label: `❌ CRM: ${msg}`,
                name: "error",
                description: "Check CRM URL and API Key",
              },
            ];
          }
          ...
          if (!result.success) {
            return [
              { label: `❌ ${result.message || "Error"}`, name: "error" },
            ];
          }
            ...
            return [
              {
                ...
                } — No data sheets configured`,
                name: "no_sheets",
                description:
                  "Go to CRM → Integration Keys → select this key → Data Sheets → enable the sheets you want.",
              },
            ];
          }
          ...
          return sheets.map((sheet: any) => ({
            ...
            description:
              sheet.description ||
              `${sheet.rowCount} rows | Last synced: ${
                sheet.lastSyncedAt || "N/A"
              }`,
          }));
        } catch (error: any) {
          return [
          ...
          return [
            {
              ...
              name: "error",
              description:
                "No RAG collections found. Create one in the ERP dashboard.",
            },
          ];
        }
  ...
  async init(
    ...
    const selectedRaw = convertMultiOptionsToStringArray(
      nodeData.inputs?.selectedSheets as string
    );
    const sheets: {
      collectionId: string;
      sheetId: number | null;
    ...
    if (sheets.length === 0) {
      throw new Error(
        "Please select at least one data sheet. Click the refresh button to load available sheets."
      );
    }
...
class AppCityRAGTool extends StructuredTool {
  name: string;
  description: string;
  ...
  schema = z.object({
    sheet: z
      ...
      .optional()
      .describe(
        'The name of the sheet to search in. E.g. "carImages", "offers", "doctors". Use "AllSheets" to search all sheets at once.'
      ),
    query: z
      .string()
      .describe("The natural language search query terms to search for."),
  });
  ...
  async _call(arg: any): Promise<string> {
    try {
      ...
      // Safe parsing & defensive type handling
      let sheetName = "";
      let query = "";

      if (arg && typeof arg === "object") {
        sheetName = arg.sheet || arg.sheetName || "";
        query = arg.query || arg.q || "";
      ...
      const isAllSheets =
        sheetName && ALL_KEYWORDS.includes(sheetName.toLowerCase().trim());
      ...
      if (!query) {
        return (
          'Please provide a search query. Example: {"sheet": "' +
   ...
   */
  private async searchSingleSheet(
    sheet: SheetConfig,
    query: string
    ...
    try {
      ...
      if (!response.ok) {
        ...
        return {
          error: `Error searching "${sheet.sheetName}": ${response.status} ${response.statusText}. ${errorText}`,
        };
      }
      ...
      if (result.status !== "ok") {
        return {
          error: `Search error in "${sheet.sheetName}": ${
  ...
  private async searchAllSheets(
    ...
    for (const { sheet, result } of results) {
      ...
      const matchedCount =
        products.length || result.data.matchedItemIds?.length || 0;
      ...
      const productList = allProducts
        .map((item, idx) => {
          ...
          const fields = Object.entries(itemData)
            .filter(
              ([key]) =>
                !["id", "embedding", "searchText", "collectionId"].includes(key)
            )
            .filter(
              ([, value]) =>
                value !== null &&
    ...
    try {
      ...
      await fetch(`${crmUrl}/api/integration/search-log`, {
        ...
        body: JSON.stringify({
          ...
          query,
          results_count:
            result.products?.length || result.matchedItemIds?.length || 0,
  ...
  private formatResults(result: any, query: string, sheetName: string): string {
    ...
    response += `\n\n[Source: ${source} | ${products.length} results${
