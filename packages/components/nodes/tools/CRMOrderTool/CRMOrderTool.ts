...
async function fetchWithRetry(
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // 5xx errors → retry
      console.warn(
        `[CRMOrderTool] Retry ${i + 1}/${retries} — server returned ${response.status}`
      );
    } catch (e: any) {
      lastError = e;
      console.warn(
        `[CRMOrderTool] Retry ${i + 1}/${retries} — ${e.message}`
      );
    }
...
// FIELD_ALIAS_MAP and normalizeAttributeKeys removed.
// The tool now trusts the LLM's key names exactly as received from the prompt.
// If the prompt says {key: "clientAddress", ...} → the tool sends clientAddress.
// If the prompt says {key: "address", ...} → the tool sends address.
// No fuzzy matching, no rewriting — the prompt is the single source of truth.
  ...
  async init(
    ...
    if (!apiKey) {
      throw new Error(
        ...
        "1. Open CRM → Integration → Orders tab\n" +
        "2. Click 'Create Key' for the client\n" +
        "3. Copy the key (starts with crm_) and paste it here"
      );
    }
...
function buildDynamicSchema(columns: any[]): z.ZodObject<any> {
  ...
  if (!columns || columns.length === 0) {
    // Minimal fallback: attributes array only, full passthrough.
    // The LLM's prompt defines the key names — this schema trusts whatever keys it sends.
    return z.object({
...
class CRMOrderToolImpl extends StructuredTool {
  ...
  // Column cache — refreshed every 5 minutes or on first call
  private columnsCache: { data: any[]; fetchedAt: number } = {
    data: [],
    fetchedAt: 0,
  };
  ...
  constructor(config: CRMOrderToolConfig) {
    ...
    // 🔥 Build the Zod schema — starts minimal, auto-enriches on first CRM fetch
    this.schema = buildDynamicSchema([]);
  }

  /**
   * Fetch CRM columns with 5-minute cache.
   * Columns are fetched lazily on first tool call, then cached.
   */
  private async getColumns(): Promise<any[]> {
    const now = Date.now();
    if (
      this.columnsCache.data.length > 0 &&
      now - this.columnsCache.fetchedAt < this.CACHE_TTL
    ) {
      return this.columnsCache.data;
    }

    try {
      const url = `${this.crmBaseUrl}/api/integration/columns`;
      const response = await fetchWithRetry(
        ...
        2 // fewer retries for column fetch
      );

      if (response.ok) {
        const result = (await response.json()) as any;
        if (result.success && result.columns?.length > 0) {
          this.columnsCache = { data: result.columns, fetchedAt: now };
          // Update schema with fresh columns
          this.schema = buildDynamicSchema(result.columns);
          return result.columns;
        }
      }
    } catch (e: any) {
      console.warn(
        `[CRMOrderTool] Failed to refresh columns: ${e.message}. Using cached data.`
      );
    }

    // Return cached data (might be empty on first call — that's OK)
    return this.columnsCache.data;
  }
  ...
  async _call(
    ...
    try {
      // ── 0. Ensure columns schema is up-to-date (lazy fetch + cache) ──
      ...
      // ── Key names are passed through exactly as the LLM (prompt) specifies ──
      // No rewriting. The prompt is the single source of truth for key names.
      ...
      // ── 7. Detailed error handling so the LLM knows what went wrong ──
      if (!response.ok) {
        if (response.status >= 500) {
          return formatToolError(
            `CRM server error (${response.status}). The order was NOT saved. Do NOT tell the customer the order was placed. Apologize and ask them to wait while you retry.`,
            payload
          );
        }
        if (response.status === 401 || response.status === 403) {
          return formatToolError(
            `CRM authentication failed (${response.status}). The integration key may be invalid or disabled.`,
            payload
          );
        }
        return formatToolError(
          `CRM rejected the request (${response.status}): ${
      ...
      return formatToolError(
        `Cannot connect to CRM at ${this.crmBaseUrl}: ${error.message}. The order was NOT saved. Ask the customer to wait briefly while you retry.`,
        {}
      );
    }
  }
