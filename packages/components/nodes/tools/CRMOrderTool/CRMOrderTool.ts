...
class CRMOrderTool_Tools implements INode {
    ...
    constructor() {
        ...
        this.description = 'Create or update orders in the CRM system via API key integration. Used by ERP Agent.'
...
- **On error**: Apologize and offer to retry`
    ...
    async init(nodeData: INodeData, _: string, _options: ICommonObject): Promise<any> {
        ...
        if (actions.includes('createOrder')) {
            ...
            tools.push(new CRMOrderToolImpl({
                crmBaseUrl,
                apiKey,
                toolName,
                toolDescription,
        ...
        if (actions.includes('updateOrder')) {
            ...
            const updateToolName = baseToolName.replace('create', 'update').replace('Create', 'Update') === baseToolName
    ...
    async _call(
        ...
        try {
            ...
            if (this.useFlowSessionId) {
                sessionIdVal = flowConfig?.sessionId || (this as any).flowObj?.sessionId
            }
            ...
            if (!response.ok) {
                return `ERROR: Failed to ${this.action} order. Status: ${response.status}. Message: ${
            ...
            if (result.success) {
                ...
                return `ERROR: ${result.message || `Failed to ${this.action} order`}. Please inform the customer about this issue.`
            }
        } catch (error: any) {
