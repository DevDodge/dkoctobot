    ...
    constructor() {
        ...
        this.inputs = [
                ...
                options: [
                    {
                        label: 'Session ID',
                        name: 'sessionId',
                        description: 'Uses $flow.sessionId – the chat session identifier from the current workflow session'
                    }
                ],
                optional: true
            },
            // ── Orders Actions ──
            {
                ...
                name: 'ordersActions',
                type: 'multiOptions',
                description: 'Select which CRM order actions to enable (defaults to Create Order if left empty)',
                options: [
                    {
                        label: 'Create Order',
                        name: 'createOrder',
...
- **On error**: Apologize and offer to retry`
                ...
                items.push({
                    ...
                    name: 'schema_info',
                    description: `The tool uses a dynamic Zod schema built from CRM columns. The AI agent will see these fields via function calling: ${columns
    ...
    async init(nodeData: INodeData, _: string, _options: ICommonObject): Promise<any> {
        ...
        let actions = convertMultiOptionsToStringArray(nodeData.inputs?.ordersActions)
        ...
        if (actions.includes('createOrder')) {
            ...
            if (!toolDescription || isPlaceholder(toolDescription)) {
                toolDescription = await getOnTheFlyDescription(crmBaseUrl, apiKey, 'create')
            }
        ...
        if (actions.includes('updateOrder')) {
            const baseToolName = (nodeData.inputs?.toolName as string) || 'create_crm_order'
            const updateToolName =
                baseToolName.replace('create', 'update').replace('Create', 'Update') === baseToolName
                    ? `${baseToolName}_update`
                    : baseToolName.replace('create', 'update').replace('Create', 'Update')

            const toolDescription = await getOnTheFlyDescription(crmBaseUrl, apiKey, 'update')
...
function isPlaceholder(desc: string): boolean {
    ...
    const placeholderPatterns = ['⚠️', 'Click refresh', 'copy the generated', 'generated prompt here', 'enter api key first']
 ...
 */
async function getOnTheFlyDescription(crmBaseUrl: string, apiKey: string, actionType: 'create' | 'update'): Promise<string> {
    try {
        const url = `${crmBaseUrl}/api/integration/columns`
        const response = await fetch(url, {
            method: 'GET',
...
function buildDynamicSchema(columns: any[]): z.ZodObject<any> {
    ...
    for (const col of columns) {
        ...
        const opts = col.options ? ` (allowed: ${typeof col.options === 'string' ? col.options : JSON.stringify(col.options)})` : ''
        ...
        switch (col.type) {
            ...
            case 'boolean':
                shape[col.key_name] = z
                    .union([z.boolean(), z.string().transform((v) => (v === 'true' || v === '1' || v === 'yes' ? true : false))])
                    .optional()
                    .describe(desc + opts)
                break
...
class CRMOrderToolImpl extends StructuredTool {
    ...
    async _call(
        ...
        try {
            ...
            if (argAny.attributes && Array.isArray(argAny.attributes) && argAny.attributes.length > 0) {
            ...
            // ── 3. Pass keys through as-is — no mapping needed ──
            // The ERP Agent already sees the EXACT CRM key names via the tool description
            // injected into its system prompt. Keys arrive here already matching CRM columns.

            // ── 4. Validate we have at least something ──
            const nonSessionAttrs = attributes.filter((a) => a.key !== 'sessionId')
            if (nonSessionAttrs.length === 0) {
                return formatToolError('No order data provided. Please provide at least customer name and phone number.', {
                    provided: attributes
                })
            }
            ...
            console.info(
                `[CRMOrderTool v2] action=${this.action} sessionId=${effectiveSessionId || '(none)'} attrs=${
                    attributes.length
                } keys=${attributes.map((a) => a.key).join(', ')}`
            )
