...
const CreateSpreadsheetSchema = z.object({
  ...
  sheetCount: z
    ...
    .optional()
    .default(1)
    .describe("Number of sheets to create"),
  locale: z
    .string()
    .optional()
    .describe("The locale of the spreadsheet (e.g., en_US)"),
  timeZone: z
    .string()
    .optional()
...
const GetSpreadsheetSchema = z.object({
  ...
  ranges: z
    .string()
    .optional()
    .describe("Comma-separated list of ranges to retrieve"),
  includeGridData: z
    .boolean()
    .optional()
    .default(false)
    .describe("True if grid data should be returned"),
});
...
const GetValuesSchema = z.object({
  ...
  majorDimension: z
    ...
    .optional()
    .default("ROWS")
    .describe("The major dimension that results should use"),
});
...
const BatchUpdateValuesSchema = z.object({
  ...
  values: z
    .string()
    .describe(
      'JSON array of value ranges to update (e.g., [{"range": "A1:B2", "values": [["A1", "B1"], ["A2", "B2"]]}])'
    ),
  includeValuesInResponse: z
    .boolean()
    .optional()
    .default(false)
    .describe("Whether to return the updated values in the response"),
});
...
class GetSpreadsheetTool extends BaseGoogleSheetsTool {
  ...
  async _call(arg: any): Promise<string> {
    ...
    try {
      ...
      const endpoint = `spreadsheets/${params.spreadsheetId}${
...
class GetValuesTool extends BaseGoogleSheetsTool {
  ...
  async _call(arg: any): Promise<string> {
    ...
    try {
      ...
      const endpoint = `spreadsheets/${
        params.spreadsheetId
      }/values/${encodedRange}${queryString ? `?${queryString}` : ""}`;
...
class UpdateValuesTool extends BaseGoogleSheetsTool {
  ...
  async _call(arg: any): Promise<string> {
    ...
    try {
      ...
      const endpoint = `spreadsheets/${
        params.spreadsheetId
      }/values/${encodedRange}?${queryParams.toString()}`;
...
class AppendValuesTool extends BaseGoogleSheetsTool {
  ...
  async _call(arg: any): Promise<string> {
    ...
    // Override spreadsheetId with appendSpreadsheetId if set
    if (this.defaultParams.appendSpreadsheetId) {
      params.spreadsheetId = this.defaultParams.appendSpreadsheetId;
    }
    ...
    try {
      ...
      const endpoint = `spreadsheets/${
        params.spreadsheetId
      }/values/${encodedRange}:append?${queryParams.toString()}`;
...
class BatchGetValuesTool extends BaseGoogleSheetsTool {
  ...
  async _call(arg: any): Promise<string> {
    ...
    try {
      ...
      const endpoint = `spreadsheets/${
        params.spreadsheetId
      }/values:batchGet?${queryParams.toString()}`;
...
export const createGoogleSheetsTools = (
