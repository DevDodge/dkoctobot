...
const SearchSchema = z.object({
    searchValue: z.string().describe('The value to search for in the configured Google Sheet columns')
})
 ...
 */
function resolveColumnIndices(searchColumns: string, headers: string[] | null): number[] {
    ...
    for (const part of parts) {
        ...
        if (/^[A-Za-z]{1,3}$/.test(part) && (!headers || !headers.includes(part))) {
            ...
            const idx = headers.findIndex((h) => h.toLowerCase() === part.toLowerCase())
            if (idx !== -1) {
                indices.push(idx)
            }
        }
 ...
 */
function matchesSearchMode(cellValue: string, searchValue: string, searchMode: string): boolean {
...
class SearchGoogleSheetTool extends DynamicStructuredTool {
    ...
    async _call(arg: z.infer<typeof SearchSchema>): Promise<string> {
        ...
        try {
            ...
            const columnIndices = resolveColumnIndices(this.config.searchColumns, headers)
            ...
            for (const row of dataRows) {
                const matches = columnIndices.some((colIdx) => {
                    ...
                    return matchesSearchMode(cellValue, searchValue, this.config.searchMode)
                })
...
export const createGoogleSheetsSearchTool = (args: { accessToken: string; config: GoogleSheetsSearchConfig }): DynamicStructuredTool[] => {
...
class GetLastRowTool extends DynamicStructuredTool {
    ...
    constructor(args: { accessToken: string; spreadsheetId: string; sheetName: string }) {
        super({
            name: 'get_last_row',
...
export const createGetLastRowTool = (args: { accessToken: string; spreadsheetId: string; sheetName: string }): DynamicStructuredTool[] => {
