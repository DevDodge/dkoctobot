...
export class App {
  ...
  async initDatabase() {
    ...
    try {
      ...
      logger.info(
        "⏹️ [server]: Abort controllers pool initialized successfully"
      );
      ...
      if (process.env.MODE === MODE.QUEUE) {
        ...
        logger.info(
          "🔗 [server]: Redis event subscriber connected successfully"
        );
      }
      ...
      if (
        ...
        try {
          ...
          this.followUpService = new FollowUpService(this.AppDataSource);
          ...
          this.followUpService.setQueue(this.followUpQueue);
          this.followUpQueue.createWorker();
          logger.info(
            "⏰ [server]: Follow-up queue and service initialized successfully"
          );
        } catch (error) {
          logger.error(
            "⚠️ [server]: Follow-up system failed to initialize (non-fatal):",
            error
          );
        }
      }
      ...
      logger.error(
        "❌ [server]: Error during Data Source initialization:",
        error
      );
    }
  }
  ...
  async config() {
    ...
    const flowise_file_size_limit =
      process.env.FLOWISE_FILE_SIZE_LIMIT || "50mb";
    this.app.use(express.json({ limit: flowise_file_size_limit }));
    this.app.use(
      express.urlencoded({ limit: flowise_file_size_limit, extended: true })
    );

    // Enhanced trust proxy settings for load balancer
    let trustProxy: string | boolean | number | undefined =
    ...
    const denylistURLs = process.env.DENYLIST_URLS
      ? process.env.DENYLIST_URLS.split(",")
      : [];
    const whitelistURLs = WHITELIST_URLS.filter(
      (url) => !denylistURLs.includes(url)
    );
      ...
      if (URL_CASE_INSENSITIVE_REGEX.test(req.path)) {
        ...
        if (URL_CASE_SENSITIVE_REGEX.test(req.path)) {
          ...
          if (isWhitelisted) {
            ...
            const isAPIKeyBlacklistedURLS = API_KEY_BLACKLIST_URLS.some((url) =>
              req.path.startsWith(url)
            );
    ...
    if (
      ...
      await this.rateLimiterManager.addRateLimiter(
        ...
        60,
        100,
        process.env.ADMIN_RATE_LIMIT_MESSAGE ||
          "Too many requests to admin dashboard, please try again later."
      );
...
export async function start(): Promise<void> {
  ...
  server.listen(port, host, () => {
    logger.info(
      `⚡️ [server]: Octobot Server is listening at ${
        host ? "http://" + host : ""
      }:${port}`
    );
  });
