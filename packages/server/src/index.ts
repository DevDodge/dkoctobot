...
export class App {
    ...
    async initDatabase() {
        ...
        try {
            ...
            if (process.env.FOLLOWUP_LEGACY === 'true' && (process.env.REDIS_HOST || process.env.REDIS_URL)) {
                try {
                    ...
                    const followUpQueueName = `${process.env.QUEUE_NAME || 'flowise-queue'}-followup`
                    this.followUpQueue = new FollowUpQueue(followUpQueueName, redisConnection, this.AppDataSource)
                    this.followUpService.setQueue(this.followUpQueue)
                    this.followUpQueue.createWorker()
                    logger.info('⏰ [server]: Follow-up queue and service initialized successfully')
                } catch (error) {
                    logger.error('⚠️ [server]: Follow-up system failed to initialize (non-fatal):', error)
                }
            }
