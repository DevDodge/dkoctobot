...
export const FlowListTable = ({
  ...
  const isActionsAvailable = isAgentCanvas
    ...
    : hasPermission(
        "chatflows:update,chatflows:delete,chatflows:config,chatflows:domains,templates:flowexport,chatflows:export"
      );
  ...
  const localStorageKeyOrderBy = getLocalStorageKeyName(
    "orderBy",
    isAgentCanvas
  );

  const [order, setOrder] = useState(
    localStorage.getItem(localStorageKeyOrder) || "desc"
  );
  const [orderBy, setOrderBy] = useState(
    localStorage.getItem(localStorageKeyOrderBy) || "updatedDate"
  );
  ...
  const onFlowClick = (row) => {
    if (!isAgentCanvas) {
      ...
      return isAgentflowV2
        ? `/v2/agentcanvas/${row.id}`
        : `/agentcanvas/${row.id}`;
    }
  };
                  ...
                  .map((row, index) => (
                    ...
                    >
                      ...
                      <StyledTableCell key="folder">
                        {row.folderId &&
                          folders.find((f) => f.id === row.folderId) && (
                            <Chip
                              label={
