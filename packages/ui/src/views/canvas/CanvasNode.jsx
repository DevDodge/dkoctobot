...
const CanvasNode = ({ data }) => {
  ...
  const getBorderColor = () => {
    ...
    else if (theme?.customization?.isDarkMode)
      return theme.palette.grey[900] + 25;
  ...
  useEffect(() => {
    ...
    if (componentNode) {
      if (!data.version) {
        ...
        setWarningMessage(
          nodeOutdatedMessage(data.version, componentNode.version)
        );
    ...
    <>
      <NodeCardWrapper
        ...
        sx={{
          ...
          ...(data.name === "monitoringPromptTemplate" && { width: "600px" }),
        }}
        border={false}
      >
        <NodeTooltip
